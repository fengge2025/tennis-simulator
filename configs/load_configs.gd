...
func load_config(file_name: String) -> ConfigFile:
	...
	var err: Error = config_file.load("res://configs/%s" % file_name)
	if err != OK:
...
func load_debug_config() -> void:
	var config_file: ConfigFile = load_config("debug.ini")
	if config_file == null:
		return

	var logger_domains: Array = config_file.get_value("logger", "logger_domains", [])
...
func load_match_config() -> void:
	var config_file: ConfigFile = load_config("match.ini")
	if config_file == null:
		return

	var territory_x_min: int = config_file.get_value("territory", "territory_x_min", 0)
	var territory_x_max: int = config_file.get_value("territory", "territory_x_max", 0)
	var territory_home_y_min: int = config_file.get_value("territory", "territory_home_y_min", 0)
	var territory_home_y_max: int = config_file.get_value("territory", "territory_home_y_max", 0)
	var territory_away_y_min: int = config_file.get_value("territory", "territory_away_y_min", 0)
	var territory_away_y_max: int = config_file.get_value("territory", "territory_away_y_max", 0)

	var home: MatchConfig = MatchConfig.new()
